...
| 5.0 | CRM Metadata-First Module | Mostly complete | Migration 0040 seeds CRM module/workspace plus five generic_json CRM DocTypes, list views, form layouts, actions, permissions, owner/admin grants, and workspace items. Browser CRUD and builder inspection passed for Leads and Opportunities. Generic list pages now reset search/filter state when switching DocTypes. Remaining gap: Check / Repair builder-home entry was still flaky under automation and needs one clean manual browser pass. |
