...
| 6.9 | Client Script Sandbox Foundation | Active | Safe client script foundation using JSON-rule DSL (not JS) for metadata-driven DocTypes. Migration 0056: erp_client_scripts table, 6 RPCs, 5 permissions. Frontend: sandbox engine, React hook, DynamicFormPage integration, management UI. CRM Lead demo script. No raw JS execution. No access to window/document/localStorage/fetch. Backend is source of truth. Not yet verified. |
