...
| 6.8.5 | Metadata Studio Module Manager Repair & Authenticated Cloud/RPC Proof | Complete (`79a01c6`) | Added Module Manager to Metadata Studio (card, sidebar, route). Full CRUD for app.erp_modules with safe-delete rules (blocked if DocTypes reference). DocType Builder shows warning when no modules exist and "Manage Modules" button. Granular module permissions granted to owner/admin. SQL migration 0055, 7 RPCs. Closeout gate v3: cloud verifier 25/25 PASS (real authenticated Supabase auth — admin sign-in + RPC calls, restricted user sign-in + RPC blocked, direct table write blocked), browser verifier 15/15 PASS (UI-only — no fake RPC calls through frontend URL). Phase 6.9 NOT started in this phase. This is NOT the future full Module Builder. |
| 6.9 | Client Script Sandbox Foundation | Complete | Migrations 0056/0057/0058 applied to Supabase Cloud. Phase 6.9.2: FK `app.companies`→`app.tenants`, `company_role_assignments` join fix, `app.current_company_id()`, `is_active` on `erp_docfields`. Phase 6.9.3: Cloud RPC contract 28/28 PASS, full cloud 36/36 PASS, RLS `.schema("app").from()` 8/8 PASS, browser SPA navigation 14/14 PASS. Phase 6.9.4: Fixed stale formValues bug in useClientScripts (setFormValues async race). Strict referral_name visibility (fail if hidden). Strict expected_value required proof (required attr, asterisk, or validation msg). ERR_NAME_NOT_RESOLVED filtered. Browser SPA 15/15 PASS. |
| X | Module Builder Foundation | Deferred / separate workspace only | Module Builder work remains outside this branch closeout and is not part of Phase 6.4/6.8.5/6.9. |
