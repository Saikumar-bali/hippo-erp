...
| 6.8.5 | Metadata Studio Module Manager Repair & Authenticated Cloud/RPC Proof | Complete (`556820e`) | Added Module Manager to Metadata Studio (card, sidebar, route). Full CRUD for app.erp_modules with safe-delete rules (blocked if DocTypes reference). DocType Builder shows warning when no modules exist and "Manage Modules" button. Granular module permissions granted to owner/admin. SQL migration 0055, 7 RPCs. Closeout gate v2: cloud verifier 25/25 PASS (real authenticated Supabase auth — admin sign-in + RPC calls, restricted user sign-in + RPC blocked, direct table write blocked), browser verifier 15/15 PASS (UI-only — no fake RPC calls through frontend URL). Phase 6.9 NOT started. This is NOT the future full Module Builder. |
