...
| 6.9 | Client Script Sandbox Foundation | Complete | Migrations 0056/0057/0058 applied to Supabase Cloud. Phase 6.9.2 honest verification: fixed FK `app.companies`→`app.tenants`, `company_role_assignments` join fix, created `app.current_company_id()`, added `is_active` to `erp_docfields`, GRANT EXECUTE + schema cache refresh in 0058. Cloud RPC contract 28/28 PASS, full cloud verifier 36/36 PASS. Browser: app home shows Client Scripts, admin/restricted login works, scripts blocked for restricted user. Typecheck/lint/build/test/simulation all pass. |
