...
| 6.9 | Client Script Sandbox Foundation | Complete | Migrations 0056/0057/0058 applied to Supabase Cloud. Phase 6.9.2: FK `app.companies`→`app.tenants`, `company_role_assignments` join fix, `app.current_company_id()`, `is_active` on `erp_docfields`. Phase 6.9.3: Cloud RPC contract 28/28 PASS, full cloud 36/36 PASS, RLS `.schema("app").from()` 8/8 PASS, browser SPA navigation 14/14 PASS. PGRST202 confirmed absent from browser. CRM Lead client script behavior verified in browser form (status→Qualified validates expected_value, source→Referral shows referral_name). Restricted user blocked for management. |
