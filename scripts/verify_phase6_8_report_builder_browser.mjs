#!/usr/bin/env node
/**
 * Phase 6.8 — Report Builder Foundation: Browser verification
 *
 * Tests:
 * 1. Admin login and Reports workspace navigation
 * 2. Reports list shows CRM Lead and CRM Opportunity reports
 * 3. Click "Run" opens report runner with columns and data
 * 4. Filters work and are applied
 * 5. Back button returns to reports list
 * 6. No page errors
 */
...
async function run() {
  console.log("\n=== Phase 6.8 Report Builder Browser Verification ===\n");
  ...
  try {
    ...
    // Find Reports group in sidebar and click it
    ...
    if (await reportsBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      await waitForAppReady(page);
      pass("2. Reports workspace item found");
    } else {
      // Try to find any button with "Report" text
      ...
      if (await altBtn.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        await altBtn.click();
        await page.waitForTimeout(2000);
        pass("2. Reports workspace item found (alt)");
      } else {
    ...
    // ── 5. CRM Opportunity report exists ──
    ...
    // ── 7. Report runner loaded with heading ──
    ...
    if (await rowCountText.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      pass(`11. Row count: "${text}"`);
    } else {
      fail("11. Row count text", "Not visible");
    }
    await screenshot(page, "11-results-data");
    ...
    if (await backBtn.isVisible({ timeout: 5000 }).catch(() => false)) {
      await backBtn.click();
      await page.waitForTimeout(2000);
      pass("12. Back to Reports");
    } else {
      fail("12. Back button", "Not visible");
    }
    await screenshot(page, "12-back-to-list");

    // ── 13. Back on reports list ──
    ...
    if (await reportsHeadingBack.isVisible({ timeout: 5000 }).catch(() => false)) {
      pass("13. Back on reports list page");
    } else {
      fail("13. Back to list", "Reports heading not visible");
    }

    // ── 14. No page errors ──
    if (errors.length === 0) { pass("14. No page errors"); } else { fail("14. No page errors", `${errors.length} errors: ${errors.slice(0, 3).join("; ")}`); }
